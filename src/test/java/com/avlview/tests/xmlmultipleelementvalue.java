...
public class xmlmultipleelementvalue extends TestBase {
	...
	public void PostAPITest() throws ClientProtocolException, IOException, SAXException, ParserConfigurationException {
		...
		String a = prop.getProperty("v1");

		String b = prop.getProperty("v2");
		String c = a + " " + b;
		...
		getvehiclesnapshotinaccountSG gnv = new getvehiclesnapshotinaccountSG(result);

		XmlMapper xmlMapper = new XmlMapper();
		String destfilepath = System.getProperty("user.dir")
				+ "\\src\\main\\java\\com\\avlview\\data\\getvehiclesnapshotinaccountSG.xml";
		xmlMapper.writeValue(new File(destfilepath), gnv);

		String resstring = xmlMapper.writeValueAsString(gnv);
		 ...
		 * System.out.println(statuscode); Assert.assertEquals(statuscode,
		 * testbase.RESPONSE_STATUS_CODE_200);
		 * 
		 * String responseStringpost = EntityUtils.toString(httpresponse.getEntity(),
		 * "UTF-8"); System.out.println(responseStringpost);
		 * 
		 ...
		 * 
		 * Header[] headersarray = httpresponse.getAllHeaders(); HashMap<String, String>
